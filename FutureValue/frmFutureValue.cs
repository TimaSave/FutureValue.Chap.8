...
namespace FutureValue
{
    public partial class frmFutureValue : Form
    {
        ...
        string[,] futurevalueCal = new string[10,4];
        int row = 0;
        ...
        private void btnCalculate_Click(object sender, EventArgs e)
        {
            try
            {
                if (IsValidData())
                {
                    ...
                    futurevalueCal[row,0] = monthlyInvestment.ToString("c");
                    futurevalueCal[row,1] = interestRateMonthly.ToString("P");
                    futurevalueCal[row,2] = years.ToString();
                    futurevalueCal[row,3] = futureValue.ToString("c");
                    row ++;
                }
            }
            catch (Exception ex)
        ...
        private void btnExit_Click(object sender, EventArgs e)
        {
            ...
            string futureVcalString = "Inv/Mo\tRate\tYears\tFutre Value\n";
            for (int i = 0; i < futurevalueCal.GetLength(0); i++)
            {
                for (int j = 0; j < futurevalueCal.GetLength(1); j++)
                {
                    futureVcalString += futurevalueCal[i, j] + "\t"; 
                        
                }
                futureVcalString += "\n";
            }
            MessageBox.Show(futureVcalString, "Calculation Test");
            this.Close();
        }
